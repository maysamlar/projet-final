...
export const getproduct = createAsyncThunk("product/get", async (req, res) => {
  try {
    let result = axios.get("http://localhost:5005/product/");
    return result;
  } catch (error) {
    console.log(error);
  }
...
export const addproduct = createAsyncThunk("product/add", async (newproduct) => {
  try {
    let result = axios.post("http://localhost:5005/product/add", newproduct);
    return result;
  } catch (error) {
    console.log(error);
  }
...
export const deleteproduct = createAsyncThunk("product/delete", async (id) => {
  try {
    let result = axios.delete(`http://localhost:5005/product/${id}`);
    return result;
  } catch (error) {
    console.log(error);
  }
...
export const editproduct = createAsyncThunk("product/edit", async ({ id, edit }) => {
  try {
    let result = axios.put(`http://localhost:5005/product/${id}`, edit);
    return result;
  } catch (error) {
    console.log(error);
  }
